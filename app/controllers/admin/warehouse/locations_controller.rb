class Admin::Warehouse::LocationsController < Admin::BaseController
  before_action :set_location, only: [:show, :edit, :update, :destroy]

  def index
    @locations = WarehouseLocation.all.order(:name)
  end

  def show
  end

  def new
    @location = WarehouseLocation.new
  end

  def create
    @location = WarehouseLocation.new(location_params)
    if @location.save
      redirect_to admin_warehouse_locations_path, notice: "Location created successfully."
    else
      render :new, status: :unprocessable_entity
    end
  end

  def edit
  end

  def update
    if @location.update(location_params)
      redirect_to admin_warehouse_locations_path, notice: "Location updated successfully."
    else
      render :edit, status: :unprocessable_entity
    end
  end

  def destroy
    @location.destroy
    redirect_to admin_warehouse_locations_path, notice: "Location deleted."
  end

  private

  def set_location
    @location = WarehouseLocation.find(params[:id])
  end

  def location_params
    params.require(:warehouse_location).permit(:name, :zone, :location_type, :capacity_volume, :capacity_weight, :is_full)
  end
end
