class Api::V1::Hrm::LeaveRequestsController < Api::V1::Hrm::BaseController
  ...
  def index
    ...
    render json: @leave_requests.as_json(
      methods: [ :number_of_days, :medical_certificate_attached, :study_timetable_attached, :medical_certificate_url, :study_timetable_url ],
  ...
  def create
    ...
    if @leave_request.save
      render json: @leave_request.as_json(methods: [ :number_of_days, :medical_certificate_attached, :study_timetable_attached, :medical_certificate_url, :study_timetable_url ]), status: :created
    else
  ...
  def update
    ...
    if @leave_request.status == "pending"
      if @leave_request.update(leave_request_params)
        render json: @leave_request.as_json(methods: [ :number_of_days, :medical_certificate_attached, :study_timetable_attached, :medical_certificate_url, :study_timetable_url ])
      else
        render json: { errors: @leave_request.errors.full_messages }, status: :unprocessable_entity
      end
    else
  ...
  def leave_request_params
    params.require(:leave_request).permit(:leave_type, :start_date, :end_date, :reason, :medical_certificate, :study_timetable)
  end
end
